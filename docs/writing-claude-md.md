...
- **200行以内**を目安に短くまとめる。長すぎると Claude がルールを無視するようになる
  （`@import` で読み込んだファイルの内容も合算されます）
- 書いた後、各行を読み返して**不要な行を削除する**。判断基準: その行を消しても Claude の動作が変わらないなら消してよい
  - 削除してよい例: `コードは読みやすく書く`（Claude はもともとそうする）
  - 残すべき例: `テスト実行: pytest tests/ -v`（これがないと Claude は正しいコマンドを知らない）
